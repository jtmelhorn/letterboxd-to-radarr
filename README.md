...
The `Publish container` workflow builds the Dockerfile and publishes to GitHub Container Registry:
...
- pull requests, as a build-only check without pushing

Tags generated by the workflow include:

- `latest` for the default branch
- the branch name
- semantic versions for `v*.*.*` tags
- `sha-<commit>` for every published build

Before publishing, confirm the repository has **Settings -> Actions -> General -> Workflow permissions -> Read and write permissions** enabled so `GITHUB_TOKEN` can write packages. After the first push, open the package at **GitHub -> Packages -> letterboxd-to-radarr -> Package settings -> Change visibility** and set it to **Public**.

Example image pull after the package is public:
